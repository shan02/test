...
class xml2dict(object):
    ...
    def _parse_node(self, node):
        ...
        for (k, v) in node.attrib.items():
            ...
            node_tree[k] = v
        #Save childrens
        for child in node:     ###################DeprecationWarning: This method will be removed in future versions.  Use 'list(elem)' or iteration over elem instead.
    ...
    def fromstring(self, s, cleanNamespace = False):
        """parse a string"""
        if cleanNamespace:
            s = re.compile(' xmlns=[^<>]*').sub('', s)
        t = ET.fromstring(s)
